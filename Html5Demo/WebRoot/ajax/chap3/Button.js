function Button(elementName, options){
    this.element = document.getElementById(elementName); //get the element node, set it into a element member.  -1
    if (!this.element) 
        throw new Error(elementName + ' not found');
    this.element.button = this;      //                   -2
    this.options = options || {};  //if options =null then set it as {}.   -3
